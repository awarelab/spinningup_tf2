...
from spinup_bis.algos.tf2.ddpg import core
...
def ddpg(env_fn, actor_critic=core.mlp_actor_critic, ac_kwargs=None, seed=0,
    ...
    critic.build(input_shape=(None, obs_dim + act_dim))

    # Target networks
    actor_targ, critic_targ = actor_critic(**ac_kwargs)

    actor_targ.build(input_shape=(None, obs_dim))
    critic_targ.build(input_shape=(None, obs_dim + act_dim))
    ...
    def target_update():
        ...
        for v, target_v in zip(critic.trainable_variables,
                               critic_targ.trainable_variables):
            target_v.assign(polyak * target_v + (1 - polyak) * v)
    ...
    def learn_on_batch(obs1, obs2, acts, rews, done):
        with tf.GradientTape(persistent=True) as g:
            # Critic update
            q = critic(tf.concat([obs1, acts], -1))
            pi_targ = actor_targ(obs2)
            q_pi_targ = critic_targ(tf.concat([obs2, pi_targ], -1))

            # Bellman backup for Q function
            backup = tf.stop_gradient(rews + gamma * (1 - done) * q_pi_targ)
            q_loss = tf.reduce_mean((q - backup) ** 2)

            # Actor update
            pi = actor(obs1)
            q_pi = critic(tf.concat([obs1, pi], -1))
            pi_loss = -tf.reduce_mean(q_pi)

        critic_grad = g.gradient(q_loss, critic.trainable_variables)
